...
| `CHRIS_DEVSTRAP_SKIP_HEAVY=1` | Skip the **heavy installs** step (`scripts/heavy-installs.sh`, last bootstrap step). Queues a manual_todo with the exact `brew bundle install --no-upgrade --file=Brewfile.heavy` command (shown in a second checklist pass after heavy). |
| `CHRIS_DEVSTRAP_HEAVY_NONINTERACTIVE=1` | Skip the Enter prompt before heavy downloads (assumes Apple ID + Mac App Store sign-in when Xcode still needs `mas`). |
| `CHRIS_DEVSTRAP_SKIP_MANUAL_GUIDE=1` | Print manual follow-ups as a list only (no Enter-to-continue). Non-TTY / CI runs behave this way automatically. |
...
**CI:** [`.github/workflows/ci.yml`](.github/workflows/ci.yml) runs `bash -n` + **ShellCheck** on Ubuntu, then a **macOS smoke** job (`/bin/bash` 3.2 syntax, remote `clt.sh` load regression, `./bootstrap.sh --dry-run`). Repo root [`.shellcheckrc`](.shellcheckrc) disables `SC1091` for dynamic `source` paths.
...
| Xcode license + first launch + iOS Simulator download | `scripts/xcode-components.sh` when Xcode was present at bootstrap start, or chained after successful `heavy-installs.sh`. Runs `sudo xcodebuild -license accept` and `xcode-select -s` Xcode.app (uses the primed sudo ticket) so new shells do not hit the license agreement error. Also pins **Xcode** to the Dock when missing (Dock step runs before heavy installs, so a just-downloaded Xcode would otherwise stay off the Dock until a re-run). |
...
| Dock | `scripts/dock.sh` — `defaults` for autohide + **tilesize / largesize 128**; `dockutil --remove all`, `config/dock-remove.txt`, `config/dock-add.tsv` (Messages, Chrome, Cursor, **Xcode**, iTerm, Notes); **`~/Downloads`** fan (**others**, date modified); `killall Dock`. Apps missing at Dock time are skipped; Xcode is re-pinned after heavy install via `xcode-components.sh`. |
...
| Heavy installs (Xcode via `mas`, Android Studio) | `scripts/heavy-installs.sh` — last step; checks `Brewfile.heavy` first (skips when satisfied). Installs Xcode and Android Studio **separately** with live `mas`/`brew` progress bars on `/dev/tty` (bootstrap’s log tee would otherwise hide them); prints wall time when each finishes (elapsed heartbeats only when no TTY). Apple ID + App Store sign-in is queued in the guided checklist only when Xcode is not already installed. Enter to start or `s` to skip; chains `xcode-components.sh` when Xcode is present. |


## Manual steps

Most setup is covered by the **guided checklist** during `./bootstrap.sh` — a first pass **before** heavy installs (iTerm, Git identity, Privacy, Raycast; Apple ID + App Store only when Xcode still needs `mas`), then a second pass **after** heavy for anything queued late (heavy skip/fail re-run command, Xcode follow-ups). Personal-only items live in [`templates/manual-overrides.example.md`](templates/manual-overrides.example.md).
...
- **Heavy installs (Xcode / Android Studio) did not run** — If Xcode is still missing, sign into Apple ID + App Store (checklist item only when Xcode needs `mas`), then press Enter at step 10 (or type `s` to skip). Re-run: `brew bundle install --no-upgrade --file=Brewfile.heavy`. Each download shows a live progress bar on the terminal (`mas` / `brew`); wall time is printed when each finishes. Xcode first-launch / Dock pin runs automatically after a successful heavy install when Xcode appears. Skip/fail re-run hints appear in the **second** checklist pass after heavy.
- **`You have not agreed to the Xcode license agreements`** — New shells after a fresh Xcode install. Current bootstrap runs `sudo xcodebuild -license accept` and `xcode-select -s` Xcode.app in `scripts/xcode-components.sh`. Fix now: `sudo xcode-select -s /Applications/Xcode.app/Contents/Developer && sudo xcodebuild -license accept` then `exec zsh`.
